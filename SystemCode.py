...
DataBaseConfiguration = {"host": "localhost", "user": "root", "password": "admin", "database": "tupcuitc"}
...
class LogInWindow(Ui_LogInWindow, QMainWindow):
    ...
    def Login(self):
        ...
        for x in range(len(LogInList)):
            if LogInList[x][0] == LogInUsername and LogInList[x][1] == LogInPassword and LogInList[x][
                2] == "Administrator":
                LogIn.hide()
                MainSystem.show()
                CreateAccount.hide()
...
class CreateAccountWindow(Ui_CreateAccountWindow, QMainWindow):
    ...
    def SignUp(self):
        ...
        for x in range(len(UsernameList)):
            if UsernameList[x][0] == Username:
                QMessageBox.warning(self, 'User Exist', 'The Username you entered already exist')
        ...
        if Password != ConfirmPassword:
            QMessageBox.warning(self, 'Create Account',
                                "The Confirm Password you entered doesn't match. Please try again.")
            self.CreateAccountCreatePasswordLineEdit.clear()
            self.CreateAccountConfirmPasswordLineEdit.clear()
            return
...
class MainSystemWindow(Ui_MainSystemWindow, QMainWindow):
    def __init__(self):
        ...
        self.MainSystemHelpPushButton.clicked.connect(self.Help)
    ...
    def QR(self):
        ...
        if self.MainSystemAccountTypeLbl.text() == "Operator":
            QMessageBox.warning(self, "Access Denied", "Only Admins are eligible to access this.")
    ...
    def Help(self):
        webbrowser.open_new(
            r'E:\DOCUMENTS\John Anthony Bataller\TUP-C [School Files]\Inventory System TUP-C UITC [GUI PY]\Help\a.pdf')
    ...
    def GenerateQRCode(self):
        ...
        if Category != "Click here to Choose Equipment Category" and Brand != "Click here to Choose Brand" and Model != "Click here to Choose Model" and NumberingStart != 0 and NumberingEnd1 != 0:
            for Numbering in range(NumberingStart, NumberingEnd2):
                qr = qrcode.make(
                    "Number:" + (str(Numbering)) + "  " + "Category: " + (str(Category)) + "  " + "Brand: " + (
                        str(Brand)) + "  " + "Model: " + (str(Model)))
                qr.save((str(Numbering)) + "  " + Category + "  " + Brand + "  " + Model + ".png")
                QMessageBox.information(self, "QR Code Generator",
                                        (str(NumberingEnd1)) + "  " + (str(Category)) + "  " + (str(Brand)) + "  " + (
                                            str(Model)) + "  " + "QR Code has been Successfully Generated.")
...
class OutItemWindow(Ui_OutItemWindow, QMainWindow):
    def __init__(self):
        ...
        self.NewBorrowerRadioButton.clicked.connect(self.EnableNewBorrower)
        self.ExistingBorrowerRadioButton.clicked.connect(self.EnableExistingBorrower)

        self.OutItemBackPushButton.clicked.connect(self.MainSystem)

    def EnableNewBorrower(self):
        self.OutItemBorrowersFirstNameLineEdit.setEnabled(True)
        self.OutItemBorrowersMiddleInitialLineEdit.setEnabled(True)
        self.OutItemBorrowersLastNameLineEdit.setEnabled(True)

        self.OutItemExistingBorrowersInfoComboBox.setDisabled(True)

    def EnableExistingBorrower(self):
        self.OutItemBorrowersFirstNameLineEdit.setDisabled(True)
        self.OutItemBorrowersMiddleInitialLineEdit.setDisabled(True)
        self.OutItemBorrowersLastNameLineEdit.setDisabled(True)

        self.OutItemExistingBorrowersInfoComboBox.setEnabled(True)
        ...
        if AddItemChecked == True:
            if ExistingEquipmentChecked == True and ExistingBrandChecked == True and ExistingModelChecked == True and ExistingManufacturerChecked == True and NoSerialNumberChecked == True:
                if EquipmentComboBoxText != "Click here to Choose Equipment" and BrandComboBoxText != "Click here to Choose Brand" and ModelComboBoxText != "Click here to Choose Model" and ManufacturerComboBoxText != "Click here to Choose Manufacturer":
                    ...
                    val = (
                    NewQuantity, EquipmentComboBoxText, BrandComboBoxText, ModelComboBoxText, ManufacturerComboBoxText,
                    NoSerialNumber)
                    cursor.execute(sql, val)
                    self.mydb.commit()

                    QMessageBox.information(self, "Data Successfully Saved",
                                            "The New Quantity of " + BrandComboBoxText + " " + ModelComboBoxText + " " + EquipmentComboBoxText + " is " + (
                                                str(NewQuantity)))
                    ...
                    QMessageBox.information(self, "Data Invalid",
                                            "Please choose EQUIPMENT, BRAND, MODEL, and MANUFACTURER.")
                ...
                if EquipmentLineEditText != "" and BrandLineEditText != "" and ModelLineEditText != "" and ManufacturerLineEditText != "":
                    ...
                    for Equipment in range(len(DBEquipmentCategory)):
                        if DBEquipmentCategory[Equipment][0] == EquipmentLineEditTextUpper:
                            QMessageBox.information(self, "Data Exist",
                                                    "The Equipment you entered already exist in the record.")
                            AddItem.ResetWindow()
                            return
                    ...
                    UQRCode = (
                                EquipmentLineEditTextUpper + BrandLineEditTextUpper + ModelLineEditTextUpper + ManufacturerLineEditTextUpper + "NoSerialNum")
                    print(UQRCode)
                    ...
                    cursor = self.mydb.cursor()
                    sql = "INSERT INTO inventory (Equipment,Brand,Model,Manufacturer,SerialNumber,Quantity,QRCode) VALUES (%s,%s,%s,%s,%s,%s,%s);"
                    val = (EquipmentLineEditTextUpper, BrandLineEditTextUpper, ModelLineEditTextUpper,
                           ManufacturerLineEditTextUpper, NoSerialNumber, QuantityValue, UQRCode)
                    cursor.execute(sql, val)
                    self.mydb.commit()
                ...
                if EquipmentComboBoxText != "Click here to Choose Equipment" and BrandLineEditText != "" and ModelLineEditText != "" and ManufacturerLineEditText != "":
                    ...
                    val = (EquipmentComboBoxText, BrandLineEditTextUpper, ModelLineEditTextUpper,
                           ManufacturerLineEditTextUpper, NoSerialNumber, QuantityValue)
                    cursor.execute(sql, val)
                    self.mydb.commit()
                ...
                if EquipmentComboBoxText != "Click here to Choose Equipment" and BrandComboBoxText != "Click here to Choose Brand" and ModelLineEditText != "" and ManufacturerLineEditText != "":
                    ...
                    sql = "INSERT INTO inventory (Equipment,Brand,Model,Manufacturer,SerialNumber,Quantity) VALUES (%s,%s,%s,%s,%s,%s);"
                    val = (
                    EquipmentComboBoxText, BrandComboBoxText, ModelLineEditTextUpper, ManufacturerLineEditTextUpper,
                    NoSerialNumber, QuantityValue)
                    cursor.execute(sql, val)
                    self.mydb.commit()
                ...
                if EquipmentComboBoxText != "Click here to Choose Equipment" and BrandComboBoxText != "Click here to Choose Brand" and ModelLineEditText != "" and ManufacturerComboBoxText != "Click here to Choose Manufacturer":
                    ...
                    cursor = self.mydb.cursor()
                    sql = "INSERT INTO inventory (Equipment,Brand,Model,Manufacturer,SerialNumber,Quantity) VALUES (%s,%s,%s,%s,%s,%s);"
                    val = (EquipmentComboBoxText, BrandComboBoxText, ModelLineEditTextUpper, ManufacturerComboBoxText,
                           NoSerialNumber, QuantityValue)
                    cursor.execute(sql, val)
                    self.mydb.commit()
                ...
                if EquipmentComboBoxText != "Click here to Choose Equipment" and BrandLineEditText != "" and ModelLineEditText != "" and ManufacturerComboBoxText != "Click here to Choose Manufacturer":
                    ...
                    sql = "INSERT INTO inventory (Equipment,Brand,Model,Manufacturer,SerialNumber,Quantity) VALUES (%s,%s,%s,%s,%s,%s);"
                    val = (
                    EquipmentComboBoxText, BrandLineEditTextUpper, ModelLineEditTextUpper, ManufacturerComboBoxText,
                    NoSerialNumber, QuantityValue)
                    cursor.execute(sql, val)
                    self.mydb.commit()
